...
**Answer:** For this instacart dataset, it contains ``r nrow(instacart)`` observations and ``r ncol(instacart)`` variables, which are respectively ``r names(instacart)`` and are 11 integer variables and 4 character variables.
...
Take the second row for example. It shows that this product is Organic 4% Milk Fat Whole Milk Cottage Cheese (ID: 1109), belongs to other creams cheeses this aisle (ID: 108), dairy eggs this department (ID: 16). The user ID of the order is 112108 and the order number is 1. This is the fourth time that this user has ordered and repurchased this product, and it is the second one added to the shopping cart of this order, and the shipping method is train.
...
```{r make a plot, fig.height=6, fig.width=10}
...
According to the plot above, we can figure out that `fresh vegetables` and `fresh fruit` are the two most popular aisles.
...
For the aisle of packaged vegetables fruits, Organic Baby Spinach, Organic Raspberries and Organic Blueberries are the three most popular items.

For the aisle of baking ingredients, Light Brown Sugar, Pure Baking Soda and Cane Sugar are the three most popular items.

For the aisle of dog food care, Snack Sticks Chicken & Rice Recipe Dog Treats, Organix Chicken & Brown Rice Recipe and Small Dog Biscuits are the three most popular items.
...
From this table, we can know the mean hour of the day at which Pink Lady Apples and Coffee Ice Cream are ordered on each day of the week while there is no different tendency for weekday and weekend.
...
brfss_location2010 = filter(brfss_smart2010, year == "2010") %>%
  ...
  unique()
```
**Answer:** According to the result of chunk above, the ``r nrow(brfss_location2002)`` states ``r unique(pull(brfss_location2002, locationabbr))`` were observed at 7 or more locations in 2002, while the ``r nrow(brfss_location2010)`` states ``r unique(pull(brfss_location2010, locationabbr))`` were observed at 7 or more locations in 2010.
...
It contains ``r nrow(Accelerometer_data)`` observations with ``r ncol(Accelerometer_data)`` variables which are respectively ``r names(Accelerometer_data)``.
...
`activity_minutes`: accelerometer measuring data for each minute in the day
...
```
**Answer:** There is a tendency that the accelerometer monitoring data in weekend is much lower than the data in weekday due to less physical activity in weekend.
...
**Answer:** According to the graph, we can figure out that the peak of activity for this male is around 12p.m and 7-9p.m.
